...
// File: contracts\CATNIPToken.sol

pragma solidity 0.6.12;


// CatnipToken with Governance.
contract CatnipToken is BEP20('Catnip Token', 'CATNIP') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "CATNIP::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "CATNIP::delegateBySig: invalid nonce");
        require(now <= expiry, "CATNIP::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "CATNIP::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying CATNIPs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "CATNIP::_writeCheckpoint: block number exceeds 32 bits");
...
// File: contracts\YoroMasterChef.sol


pragma solidity 0.6.12;


// YoroMasterChef is the master of Catnip. He can make Catnip and he is a fair guy.
//
...
contract YoroMasterChef is Ownable {
    using SafeMath for uint256;
    ...
    // Bonus muliplier for early catnip makers.
    ...
    // Update the given pool's CATNIP allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 catnipReward = multiplier.mul(catnipPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        catnip.mint(devaddr, catnipReward.div(10));
    ...
    // Deposit LP tokens to MasterChef for CATNIP allocation.
    ...
    //Catnip has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _catnipPerBlock) public onlyOwner {
        massUpdatePools();
        catnipPerBlock = _catnipPerBlock;
    }
}